...
- **设备后台静默监控（桌面版）**：右键设备「设备监控（静默采集）…」配置 SSH/Telnet 账号与命令后，后台定时采集命令输出并写入本地日志（**按天归档**：`monitor-logs/设备名/日期/<设备名>_<管理口>.log`，同一天连接/重连只追加同一文件）。每个管理口可单独设置**连接时执行命令**（每行一条可多条，每次连接成功仅执行一次）、**在线探测**（TCP/ICMP，离线变红并弹通知）、**输出关键字告警**（多关键字同时命中全部显示，**全部不再命中才解除**，告警事件携带具体匹配行）与**配置自动备份**（命令可多条合并保存；连接方式可选**复用监控连接 / 独立连接**；首份备份显示「首次」而非「有变化」）；**仅读取模式**同样支持探测 / 告警 / 备份，**仅探测模式**可不勾仅读取、不填命令、只勾选「在线探测」即只做连通性监测。「监控日志…」支持**全局跨文件搜索**（一次搜全部设备 / 日期 / 文件，点击结果定位到行）
- **监控中心（桌面版）**：工具栏「监控 ▾ 监控中心…」或右键设备进入，**聚合全部监控状态**（在线/离线/告警/备份统计）、**事件时间线**（离线/恢复/告警/告警解除/备份变化，事件带设备徽标与具体告警内容）与**配置备份一览**——「事件时间线 / 配置备份」以**标签页**切换，**点击设备名或管理地址可筛选时间线**，打开期间实时刷新
