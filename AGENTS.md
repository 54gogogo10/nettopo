...
node test/run-tests.js             # 单元测试（纯 Node，当前 2061 项；改动后必跑且须全绿）
...
node test/smoke-shell.js           # Electron 冒烟（需桌面环境）：另有 smoke-backup / smoke-center / smoke-monitor / smoke-cred（统一凭据库）/ smoke-alertdeps（告警依赖抑制）/ smoke-backupignore（配置变更忽略规则）/ smoke-teampack（团队基线包）/ smoke-sla（可用性报表）/ smoke-l2（二层推断）
