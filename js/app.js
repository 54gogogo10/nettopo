...
function openHelp() {
  ...
  <div class="help-body">
    ...
    <table>
      ...
      <tr><td>聚合组</td><td>eth-trunk / lag / aggregate</td><td>可选（链路聚合，见⑥）</td></tr>
    </table>
    <p>无表头按「设备A, 设备B, 接口A, IP A, 接口B, IP B, 带宽, 备注」顺序识别；自动识别 UTF-8 / GBK 编码，兼容 .csv / .txt / .xlsx / .xls。</p>
    <h4>③ 从邻居表导入（LLDP / CDP）</h4>
    <ul>
      <li><b>粘贴解析</b>：在设备上执行 <b>display lldp neighbor</b>（华为/H3C，支持 brief 简表、详细键值块、「接口 has N neighbor(s)」段头、H3C verbose）或 <b>show cdp neighbors</b> / <b>show lldp neighbors</b>（思科表格 / detail 块），输出原样粘贴后点「解析预览」，确认后合并进拓扑</li>
      <li><b>从设备采集（桌面版）</b>：弹窗内直接自动登录取回邻居表——连接参数预填自该设备的监控配置（也可手填），厂家命令预设可选（自动尝试 / 华为 / H3C / 思科 / 锐捷），分页提示自动空格翻页，Telnet 自动应答登录</li>
    ...
    <ul>
      ...
      <li><b>多选</b>：Ctrl 点选、Shift 拖拽框选；多选后可整体拖动、批量编辑、对齐 / 分布、批量删除</li>
      <li><b>快速搜索</b>：<kbd>Ctrl+F</kbd> 按设备名 / 类型 / 管理地址 / 接口 / IP / VLAN 即输即搜，Enter / ↑ / ↓ 循环定位</li>
    ...
    <ul>
      ...
      <li><b>监控 / AI</b>（桌面版）：监控中心、设备监控、诊断工具箱、MAC/ARP 终端定位、网络服务、合规检查、AI 助手等，见⑪~⑯</li>
    </ul>
    <h4>⑥ 设备、连线与接口总表</h4>
    <ul>
      <li><b>设备</b>：可配置多个管理口地址（编辑设备点「＋ 增加管理口」）、管理 Web 页 URL、型号 / 软件版本 / 厂家 / 设备级图标与备注；「编辑 ▾ 类型管理」自定义类型并上传图片</li>
      <li><b>连线</b>：接口 / IP / 掩码 / VLAN / 带宽 / 备注；带宽按 Mbps 存色（100M 灰 / 1G 蓝 / 10G 紫 / 40G 橙 / 100G 红）；右键连线可<b>标记故障</b>（模拟断链，路径分析自动绕行）</li>
      <li><b>链路聚合</b>：编辑连线填「聚合组」（如 Eth-Trunk1 / Port-Channel1）或多选连线批量设置；同一对设备间同名聚合组的平行链路视为一条聚合链路——校验不再提示平行链路、路径分析按<b>成员带宽之和</b>计算、标注第三行显示聚合名并同步导出</li>
      <li><b>接口总表</b>（编辑 ▾）：全部链路两端接口集中成一张可筛选表格，修改接口 / IP / 掩码 / VLAN / 聚合组 / 二层后「应用修改」一次写入（一个撤销步骤），支持导出 CSV——批量改 IP 不用逐台双击</li>
    </ul>
    <h4>⑦ 分析与校验</h4>
    <ul>
      <li><b>路径分析</b>：选两台设备按带宽优选最宽路径并高亮（显示瓶颈带宽），故障链路自动绕行</li>
      <li><b>网段分析</b>（布局 ▾）：全部接口 IP / VLAN 接口 / 管理地址按网段汇总利用率与 VLAN，自动检测<b>网段重叠 / 网段·广播地址误用 / 超容量</b>，点击行定位并高亮成员，可导出 CSV</li>
      <li><b>拓扑校验</b>：一键检查重复 IP / 接口 / 管理地址、孤立设备、环路、平行链路、跨网段等，报告内点击定位到画布</li>
      <li><b>单点故障分析</b>（布局 ▾）：找出单点设备与无冗余关键链路，红色高亮受影响设备；右键设备 / 连线「故障影响分析…」模拟任一单点故障，查看失联区域或最短绕行路径</li>
      <li><b>对比工程</b>（文件 ▾）：另一份工程 / 表格与当前拓扑做差异对比（新增 / 删除 / 变更）</li>
    </ul>
    <h4>⑧ 保存与导出</h4>
    <ul>
      <li><b>工程文件 .nettopo</b>：保存 / 打开含位置、视图、自定义类型、多管理口、监控配置的完整工程，可选<b>口令加密</b>（PBKDF2 + AES-GCM）</li>
      <li><b>CSV / Excel</b>：把拓扑保存回连线关系表（多管理口、聚合组列，可再导入回环）</li>
      <li><b>PDF 矢量</b>高清交付；<b>PNG / SVG</b> 图片导出与一键复制到剪贴板；<b>Visio .vsdx</b> 原生格式可在 Visio 中继续编辑</li>
      <li><b>资产清单（Excel）</b>：设备台账含监控状态与备份概况；<b>IP 规划清单</b>：全部管理口与接口 IP 附对端信息；<b>设计报告</b>：自包含 HTML</li>
      <li><b>生成设备配置</b>：华为 / H3C / 思科 / 锐捷及自定义模板（{name} {mgmt} {iface} {ip} {peer} {vlan}…），生成前自动<b>冲突检查</b>，可下载 ZIP 按厂家分目录；掩码变量三种写法：{mask}（255.255.255.0）/ {maskCidr}（/24）/ {wildcard}（0.0.0.255）</li>
    </ul>
    <h4>⑨ Web Shell（桌面版）</h4>
    <ul>
      <li><b>连接</b>：右键设备「Web Shell（SSH/Telnet）…」，<b>独立窗口多标签</b>管理，主界面不锁定；SSH 支持密码 / 公钥 / keyboard-interactive 与<b>跳板机（堡垒机）</b>转发，主机指纹 SHA256 展示、首连信任变化拒连</li>
      <li><b>编码与书签</b>：输出编码可选 UTF-8 / GBK（老设备中文不乱码）；连接可「保存为书签」（密码经系统级加密），顶栏「☆ 书签」双击即连；关窗重开自动<b>恢复上次的标签连接</b>，断连后原地重建不丢终端历史</li>
      <li><b>终端效率</b>：选中即复制、Ctrl+Shift+C/V、字号调节；底部<b>快捷按钮条</b>（支持 \n 回车、\t 制表、\p 暂停，可插 {ip}/{hostname} 等设备变量）；<kbd>Ctrl+F</kbd> 终端内即输即搜；<kbd>Ctrl+P</kbd> 快速命令面板（按钮 / 书签 / 历史命令模糊搜索）</li>
      <li><b>群发</b>：顶栏「⇉ 群发」向多标签广播命令（支持 {ip} 等变量模板），并可<b>对比</b>各标签输出、高亮差异行</li>
      <li><b>SFTP 文件面板</b>（顶栏「⇅ 文件」）：浏览当前 SSH 会话远程目录，上传 / 下载 / 新建目录 / 重命名 / 删除，本地保存路径走系统对话框（Telnet 不支持）</li>
      <li><b>录制 / 回放</b>：顶栏「⏺ 录制」把终端输出录成 JSONL 录像，「▶ 回放」在只读终端按 0.5~8 倍速回放（键入不回放，避免敏感信息二次显示）</li>
      <li><b>AI 助手</b>（顶栏「✦ AI 助手」）：自然语言生成命令，设备类型下拉注入提示词，执行权限三档（确认后执行 / 仅填入不执行 / 直接执行），详见⑯</li>
    </ul>
    <h4>⑩ 设备管理 Web 页（桌面版）</h4>
    <p>设备编辑中配置「管理Web页URL」，右键设备「打开设备管理页面」在<b>独立窗口</b>以<b>多标签</b>打开；支持地址栏 / 后退 / 前进 / 刷新与页面缩放记忆；HTTPS 自签名 / 无效证书会弹出<b>安全告警</b>，手动确认后可继续访问并记住该站点。</p>
    <h4>⑪ 后台监控（桌面版）</h4>
    <ul>
      <li><b>入口</b>：右键设备「设备监控（静默采集）…」配置协议 / 管理口 / 账号 / 命令与循环间隔，后台静默经 SSH/Telnet 定时采集；全部输出带时间戳写入本地日志（<code>userData/monitor-logs/设备名/日期/</code>，按天归档，同日重连只追加同一文件）</li>
      <li><b>连接时执行命令</b>：每行一条可多条，每次连接成功仅执行一次、先于周期循环（会话初始化用）</li>
    ...
    <h4>⑮ 配置合规基线检查（桌面版）</h4>
    <ul>
      <li><b>入口</b>：「监控 ▾ 配置合规检查…」对配置备份库各地址的<b>最新配置</b>执行本地规则扫描（纯本机，不上传）</li>
      <li><b>模板</b>：内置 5 套基线——等保通用（11 条，推荐）/ 最小基线 / 华为 VRP / 思科 IOS / 接入层交换机；当前规则可<b>另存为模板</b>多套自定义（同名覆盖、可删除）</li>
      <li><b>规则</b>：按「时间同步 / 日志审计 / 认证与授权 / 服务与协议 / 路由与网关」五组，逐行正则（必须存在 / 禁止出现）；禁止类自动排除 undo / no 关闭命令与 stelnet 误命中</li>
      <li><b>结果</b>：违规定位到具体配置行，一键导出报告（Excel / CSV）；监控「自动合规」在每次备份后按当前规则集自动扫描</li>
    </ul>
    <h4>⑯ AI 助手（桌面版）</h4>
    <ul>
      <li><b>AI 设置…</b>（AI ▾）：接入 OpenAI 兼容接口与 Anthropic Claude 协议，内置知名供应商预设（DeepSeek / 智谱 GLM / 通义千问 / Kimi / Ollama 本地等，选中自动填充地址），可一键拉取服务端<b>模型列表</b>下拉选择；API Key 经系统加密仅存本机</li>
      <li><b>解析设备配置 / 日志</b>：LLM 解读配置备份与监控 / Syslog 日志，流式输出、可附加要求（如「重点检查 ACL」），结果自动存入<b>分析记录</b>可回看导出</li>
      <li><b>Shell 命令助手</b>：Web Shell 内自然语言生成命令（设备类型下拉注入语法提示词，可附带终端输出作上下文），执行权限三档：<b>确认后执行</b>（默认）/ 仅填入不执行 / 直接执行；支持生成<b>配置段</b>（完整变更序列含进入 / 退出配置模式）</li>
      <li><b>合规修复建议</b>：合规报告一键汇总违规清单，生成分设备修复命令序列（无法自动修复的标注需人工评估）</li>
      <li><b>巡检日报</b>：监控中心一键汇总快照生成中文日报；「AI ▾ 巡检日报定时…」可设每日自动生成并存入分析记录</li>
      <li><b>隐私边界</b>：AI 为唯一需外网调用的功能——发送内容为所选配置 / 日志（分隔符包裹声明为不可信数据，防提示注入）；不配置 AI 服务则完全无外网流量</li>
    </ul>
    <h4>快捷键</h4>
    <table>
